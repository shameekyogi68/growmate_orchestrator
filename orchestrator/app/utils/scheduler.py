...
async def queue_scheduled_notification(
    user_id: int, title: str, body: str, scheduled_at: datetime, data: dict = None
):
...
async def process_scheduled_queue():
    ...
    for note in pending:
        try:
            ...
            logger.error(
                f"[Scheduler] [Queue] Failed to process notification {note['id']}: {e}"
            )
    ...
    for user in users:
        ...
        milestones = {
            1: {
                "kn": (
                    "ಬೆಳೆ ಕಾಳಜಿ 🌾",
                    f"ನಮಸ್ಕಾರ {name}, ಸಸಿಗಳು ಮೊಳಕೆಯೊಡೆಯುತ್ತಿವೆ. ಈಗ ಕ್ಷೇತ್ರಕ್ಕೆ ಭೇಟಿ ನೀಡಿ ಕಳೆಗಳನ್ನು ಗಮನಿಸಿ.",
                ),
                "en": (
                    "Crop Care 🌾",
                    f"Hi {name}, seedlings are emerging. Visit your field today to check for weeds.",
                ),
            },
            15: {
                "kn": (
                    "ಗೊಬ್ಬರ ನೀಡುವ ಸಮಯ 🚜",
                    f"{name}, {crop} ಬೆಳೆಗೆ ಮೊದಲ ಸುತ್ತಿನ ಗೊಬ್ಬರ ಸೇರಿಸುವ ಸಮಯ ಬಂದಿದೆ.",
                ),
                "en": (
                    "Fertilizer Alert 🚜",
                    f"{name}, it's time for the first round of fertilizer for your {crop}.",
                ),
            },
            45: {
                "kn": (
                    "ಕೀಟಗಳ ಎಚ್ಚರಿಕೆ 🐛",
                    f"{name}, ನಿಮ್ಮ ಬೆಳೆಯನ್ನು ಕೀಟಬಾಧೆಯಿಂದ ರಕ್ಷಿಸಲು ಎಚ್ಚರದಿಂದಿರಿ.",
                ),
                "en": (
                    "Pest Monitoring 🐛",
                    f"{name}, keep an eye out for pests on your {crop} to ensure a healthy harvest.",
                ),
            },
        }
        ...
        if das in milestones:
            ...
            if not exists:
                await queue_scheduled_notification(
                    user["id"], title, body, scheduled_time, {"das": das, "crop": crop}
                )
                logger.info(
                    f"[Scheduler] [Lifecycle] Queued DAS {das} tip for user {user['id']}"
                )
    ...
    for crop, variety in TOP_CROPS:
        try:
            ...
            await cache_client.set_cached_advisory(
                f"market:{crop}:{variety}", data, ttl_seconds=ttl
            )
...
async def start_scheduler():
    ...
    scheduler.add_job(
        process_scheduled_queue,
        trigger=IntervalTrigger(minutes=1),
        id="notif_queue_processor",
        replace_existing=True,
    )

    # 4. Lifecycle Intelligence (Daily at 7 AM)
    scheduler.add_job(
        generate_crop_lifecycle_notifications,
