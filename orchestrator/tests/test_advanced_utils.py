...
async def verify_advanced_polish():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            print(f"   - Mandi Tracking: {len(mandi.get('markets', []))} local markets found")
            ...
            print(f"   - Weather: {weather.get('condition')} ({weather.get('temperature')}°C)")
            ndvi = ui.get("satellite_monitoring", {})
            print(f"   - NDVI Health: {ndvi.get('health_index')} ({ndvi.get('condition')})")
